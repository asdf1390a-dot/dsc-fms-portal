// pages/assets/quick-update.js
//
// Phase 3 — field-friendly quick update form.
//
// Flow:
...
export default function QuickUpdatePage() {
  ...
  useEffect(() => {
    if (!authLoading && !isAuthed) {
      router.replace(`/login?next=${encodeURIComponent(router.asPath || '/assets/quick-update')}`);
    }
  ...
  async function handleSave(e) {
    ...
    if (typeof navigator !== 'undefined' && navigator.onLine) {
      try {
        ...
        if (j?.asset) {
          setAsset((prev) => ({ ...prev, ...j.asset, name: prev?.name }));
        }
        setNote('');
        return;
      } catch (err) {
  ...
  return (
    <>
      <Head>
        <title>{t('quick.title', language)} · DSC FMS</title>
      </Head>
