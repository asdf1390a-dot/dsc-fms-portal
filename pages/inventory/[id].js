...
export default function InventoryDetailPage() {
  ...
  const refetch = useCallback(async () => {
    ...
    const { data: partRow, error: partErr } = await supabase
      .from('spare_parts')
      .select('id, part_number, name_ko, name_en, category, asset_id, quantity, min_quantity, unit, location, notes, created_at')
      .eq('id', id)
      .maybeSingle();
    ...
    const { data: moveRows } = await supabase
      ...
      .select('id, movement_type, quantity, reason, performed_by, performed_at')
      .eq('part_id', id)
              ...
              <Section title={activePanel === 'IN' ? '입고 처리' : '출고 처리'}>
                ...
                <div style={S.fieldGap}>
                  ...
                  <input
                    ...
                    placeholder={activePanel === 'IN' ? '예: 신규 입고, 반품' : '예: BM 사용, 점검 교체'}
                    style={S.input}
                  />
                </div>
                <div style={S.fieldGap}>
