    ...
    <>
      ...
      <div style={S.page}>
        <div style={S.wrap}>
          ...
          <div style={S.card}>
            <div style={S.cardTitle}>생산현황 파일 업로드</div>
            <div style={S.hint}>생산현황_N월_RevX.xlsx</div>
          ...
          {extracted && (
            <div style={S.card}>
              <div style={S.cardTitle}>추출된 생산 데이터</div>
              {extracted.byModel && (
                <table style={S.table}>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {extracted.byModel.map((row, i) => (
                      <tr key={i}>
                        ...
                        <td style={{...S.td, textAlign:'right'}}>{row.plan?.toLocaleString()}</td>
                        <td style={{...S.td, textAlign:'right', color:'#38bdf8'}}>{row.actual?.toLocaleString()}</td>
              ...
              {extracted.efficiency !== undefined && (
                <div style={S.summary}>
                  <span>종합생산성: <b style={{color:'#38bdf8'}}>{(extracted.efficiency * 100).toFixed(1)}%</b></span>
                  {extracted.directWorkers && <span>직접인원: <b>{extracted.directWorkers}명</b></span>}
                </div>
              )}
            </div>
          )}
...
const S = {
  ...
  cardTitle: { fontSize:14, fontWeight:700, marginBottom:8, color:'#94a3b8' },
  hint: { fontSize:12, color:'#475569', marginBottom:8 },
  ...
  summary: { display:'flex', gap:16, fontSize:13, color:'#e2e8f0', marginTop:6 },
