...
const ITEMS: Item[] = [
  ...
  { href: '/travels',   label: '여행',  match: (p) => p === '/travels' || p.startsWith('/travels/'),          icon: PlaneIcon },
...
export default function BottomNavApp() {
        ...
        {ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              style={{ ...S.item, color: active ? '#ef4444' : '#94a3b8' }}
            >
              <Icon />
              <span
                style={{
